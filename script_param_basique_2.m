...
%---Param??tres du RBDE
ini_R=[zeros(1,nIni);rand(1,nIni)];
imp_R=[rand(1,nImp);zeros(1,nImp)];
inh_R=[rand(1,nInh);zeros(1,nInh)];
act_R=[ones(2,nAct)];
%% Vecteurs des paramètres connus
%---Paramètres de départ
ini0=[zeros(1,nIni);zeros(1,nIni)+0.01];
imp0=[zeros(1,nImp)+0.01;zeros(1,nImp)];
inh0=[zeros(1,nInh)+0.01;zeros(1,nInh)];
act0=ones(2,nAct);
%act0=zeros(2,nAct)+0.1;
par0={ini0 imp0 inh0 act0};
%---Paramètres connus (négatif : paramètres inconnus)
iniKn=[ones(1,nIni);zeros(1,nIni)];
impKn=[zeros(1,nImp);ones(1,nImp)];
inhKn=[zeros(1,nInh);ones(1,nInh)];
actKn=ones(2,nAct);
